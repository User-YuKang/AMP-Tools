...
int main(int argc, char** argv) {
    ...
    //     std::vector<double> links = std::vector<double>{length(re), length(re), length(re)};
    //     Eigen::Vector3d state = Eigen::Vector3d(angle(re), angle(re), angle(re));
    //     MyManipulator2D manipulator(links);

    //     Eigen::Vector2d end_point = manipulator.getJointLocation(state, 3);

    //     Eigen::Vector3d result_state = manipulator.getConfigurationFromIK(end_point);
    //     Eigen::Vector2d test_end_point = manipulator.getJointLocation(result_state, 3);
    ...
    // Grade method
    amp::HW4::grade<MyManipulator2D>(cspace_constructor, "yu.kongc@colorado.edu", argc, argv);
    return 0;
}