    ...
    for (int num_it = 0; num_it < iteration; num_it++){
        if(double(rand())/RAND_MAX < bias){
            ...
            do{
                ...
                if(new_point.norm() < pow(10, -6)){
                    break;
                }
            }while((goal_ - new_point).norm() > pow(10,-6));
            
            if((goal_ - new_point).norm() < pow(10,-5)){
                success = true;
                break;
            }
        ...
        extendRRT(rand_point, collision_checker_);
    }
    amp::Path path;
    if(success){
        path.waypoints.push_back(goal_);
...
Eigen::VectorXd MyGenericRRT::generatePoint(const std::vector<std::pair<double, double>>& bounds){
    ...
    for(int i = 0; i < bounds.size(); i++){
        ...
        point(i) = lower + (upper - lower)*rand()/RAND_MAX;
    }
    return point;
}
...
Eigen::VectorXd MyGenericRRT::extendRRT(const Eigen::VectorXd& point, BaseCollisionChecker<Eigen::VectorXd>& collision_checker_){
    ...
    if(one_2_two.norm() < step_size){
        step = one_2_two;
    } else{
        step = one_2_two.normalized()*step_size;
    }

    if(step.norm() < pow(10,-6)){
        return step;
    }
