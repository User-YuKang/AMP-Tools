...
MyAStarAlgo::GraphSearchResult MyAStarAlgo::search(const amp::ShortestPathProblem& problem, const amp::SearchHeuristic& heuristic) {
    ...
    std::map<amp::Node, std::pair<amp::Node, double>> open_set;
    open_set[problem.init_node] = std::pair<amp::Node, double>(-1,-1);
    std::map<amp::Node, std::pair<amp::Node, double>> closed_set;
    ...
    while(open_set.size() > 0){
        ...
        LOG("round: " << round++);
        for(std::map<amp::Node, std::pair<amp::Node, double>>::iterator it = open_set.begin(); it != open_set.end(); ++it){
        ...
        std::vector<amp::Node> children = graph.children(n_best);
...
double MyAStarAlgo::backPointerLength(const amp::ShortestPathProblem& problem, const std::map<amp::Node, std::pair<amp::Node, double>>& closed_set, amp::Node parent, amp::Node child){
    double length = 0;
...
double MyAStarAlgo::edgeLength(const amp::ShortestPathProblem& problem, amp::Node parent, amp::Node child){
    amp::Graph<double> graph = *problem.graph;
    std::vector<amp::Node> children = graph.children(parent);
    std::vector<double> edge = graph.outgoingEdges(parent);
